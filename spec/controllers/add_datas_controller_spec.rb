...
describe Api::V1::AddDatasController do
  ...
  context "with valid input" do
    ...
    it "create the record data" do
      ...
      post :create, {body: {user: user1.email,device: device.uuid, record_table: [{hash_key: Faker::Lorem.characters(20), amount_to_main: 50.5},{hash_key: Faker::Lorem.characters(20), amount_to_main: 50.5}]}.to_json}
    ...
    it "create the payee data" do
      user1 = Fabricate(:user)
      device = Fabricate(:device, user_id: user1.id)
      post :create, {body:{user: user1.email, device: device.uuid, payee_table: [{hash_key: Faker::Lorem.characters(20),payee_name: "Mary"}]}.to_json}
    ...
    it "create the currency data" do
      user1 = Fabricate(:user)
      device = Fabricate(:device, user_id: user1.id)
      post :create, {body: {user: user1.email, device: device.uuid, currency_table: [{currency_code: Faker::Lorem.characters(3), rate: 3.9}]}.to_json}
    ...
    it "create the payment data" do
      user1 = Fabricate(:user)
      device = Fabricate(:device, user_id: user1.id)
      post :create, {body: {user: user1.email, device: device.uuid, payment_table: [{hash_key: Faker::Lorem.characters(20), payment_name: "Bank"}]}.to_json}
    ...
    it "create the pref data" do
      user1 = Fabricate(:user)
      device = Fabricate(:device, user_id: user1.id)
      post :create, {body: {user: user1.email, device: device.uuid, pref_table: [{key: Faker::Lorem.characters(20),value: "b"}]}.to_json}
    ...
    it "create the project data" do
      user1 = Fabricate(:user)
      device = Fabricate(:device, user_id: user1.id)
      post :create,{body: {user: user1.email, device: device.uuid, project_table: [{hash_key: Faker::Lorem.characters(20), project_name: "Fat"}]}.to_json}
