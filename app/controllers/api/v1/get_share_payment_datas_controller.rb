class Api::V1::GetSharePaymentDatasController < ApplicationController
  ...
  def users_who_shared_by_owner
    ...
    if user && params[:payment_hash_key]
      relations = UserSharePaymentRelation.joins(:share).where("owner_user_id is ? and payment_hash_key is ? and is_approved = ?", user.id, params[:payment_hash_key], true).select("users.id,users.email")
